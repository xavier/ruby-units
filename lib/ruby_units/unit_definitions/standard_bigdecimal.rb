...
Unit.define('inch') do |inch|
  inch.definition = Unit(BigDecimal(".0254"), 'meter')
...
Unit.define('survey-foot') do |sft|
  sft.definition = Unit(BigDecimal(".30480061"),'meter')
...
Unit.define('furlong') do |furlong|
  furlong.definition = Unit(BigDecimal("0.125"),'mile')
  furlong.aliases    = %w{fur furlong furlongs}
end

Unit.define('rod') do |rod|
  rod.definition  = Unit(BigDecimal('16.5'), 'feet')
...
Unit.define('mil') do |mil|
  mil.definition = Unit(BigDecimal('0.001'),'inch')
  mil.aliases    = %w{mil mils}
end

Unit.define('angstrom') do |ang|
  ang.definition = Unit(BigDecimal('0.1'),'nm')
...
Unit.define('point') do |point|
  point.definition = Unit(BigDecimal('0.013888889'),'ft')
...
Unit.define('carat') do |carat|
  carat.definition = Unit(BigDecimal("0.0002"),'kg')
...
Unit.define('liter') do |liter|
  liter.definition = Unit(BigDecimal("0.001"),'m^3')
...
Unit.define('gee') do |gee|
  ...
  gee.definition    = Unit(BigDecimal('9.80665'), 'm/s^2') # equivalent to 9.80665 m/s^2
...
Unit.define('dyne') do |dyne|
  dyne.definition = Unit(BigDecimal('0.00001'),'N')
...
temp_convert_factor = BigDecimal("0.555555556")
...
Unit.define('tempF') do |tempF|
  ...
  tempF.to_base   = lambda {|scalar| (scalar+459.67)*temp_convert_factor }
  tempF.from_base = lambda {|scalar| (scalar * BigDecimal("1.8") - 459.67)}
end

Unit.define('tempR') do |tempR|
  tempR.definition  = Unit('1 tempF')
  tempR.temperature_scale = 'rankine'
  tempR.to_base   = lambda {|scalar| scalar*temp_convert_factor}
  tempR.from_base = lambda {|scalar| scalar*BigDecimal("1.8") }
end
...
Unit.define('mmHg') do |mmhg|
  density_of_mercury  = Unit(BigDecimal('13.5951'), 'g/cm^3') # 13.5951 g/cm^3 at 0 tempC
...
Unit.define('torr') do |torr|
  torr.definition = Unit(BigDecimal('0.001315789'),'atm')
...
Unit.define('btu') do |btu|
  btu.definition  = Unit(BigDecimal('1055.056'),'J') # 1055.056 J  --- ISO standard
...
Unit.define('calorie') do |calorie|
  calorie.definition  = Unit(BigDecimal('4.184'), 'J')
...
Unit.define('unit') do |unit|
  unit.definition   = Unit(BigDecimal('0.016666667'), 'microkatal')
...
Unit.define('bit') do |bit|
  bit.definition  = Unit(BigDecimal('0.125'),'byte')
  bit.aliases     = %w{b bit}
end

#currency
Unit.define('cents') do |cents|
  cents.definition  = Unit(BigDecimal('0.01'), 'dollar')
end
...
Unit.define('percent') do |percent|
  percent.definition  = Unit(BigDecimal('0.01'))
